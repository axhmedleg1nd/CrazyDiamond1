...
public final class StandActions {
	...
	private static final DustParticleEffect PINK = new DustParticleEffect(new Vector3f(1.0f, 0.45f, 0.65f), 1.2f);
	private static final DustParticleEffect ICE = new DustParticleEffect(new Vector3f(0.55f, 0.82f, 1.0f), 1.2f);
	...
	static void barrage(ServerWorld world, ServerPlayerEntity owner, int elapsed, int remaining) {
		...
		for (int i = 0; i < count; i++) {
			...
			target.velocityModified = true;
			world.spawnParticles(ParticleTypes.CRIT, target.getX(), target.getBodyY(0.5), target.getZ(), 6, 0.35, 0.35, 0.35, 0.3);
			world.spawnParticles(PINK, target.getX(), target.getBodyY(0.5), target.getZ(), 4, 0.4, 0.4, 0.4, 0.0);
			world.spawnParticles(ICE, target.getX(), target.getBodyY(0.5), target.getZ(), 4, 0.4, 0.4, 0.4, 0.0);
		}
	...
	/** Streaks of pink/blue fists flying forward, plus the "ORA ORA" text. */
	private static void barrageFx(ServerWorld world, ServerPlayerEntity owner, int elapsed) {
		Vec3d look = owner.getRotationVec(1.0f);
		Vec3d origin = owner.getEyePos().add(0.0, -0.4, 0.0);
		for (double d = 0.8; d <= REACH; d += 0.6) {
			Vec3d p = origin.add(look.multiply(d));
			world.spawnParticles(((int) (d * 10)) % 2 == 0 ? PINK : ICE, p.x, p.y, p.z, 2, 0.35, 0.3, 0.35, 0.0);
		}
		if (elapsed % 4 == 0) {
			String ora = "ОРА ".repeat(1 + (elapsed / 4) % 4).trim();
			owner.sendMessage(Text.literal(ora).formatted(Formatting.LIGHT_PURPLE, Formatting.BOLD), true);
		}
	}
