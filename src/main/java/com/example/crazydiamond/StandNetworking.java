...
public final class StandNetworking {
	...
	/** If true, a player needs a Stand Arrow (or creative mode) before they can summon the stand. */
	public static boolean REQUIRE_STAND_ARROW = true;
	...
	private static void handle(ServerPlayerEntity player, int action) {
		if (action >= DvizhukhaAbility.ACTION_BASE) {
			int index = action - DvizhukhaAbility.ACTION_BASE;
			DvizhukhaAbility[] dvizh = DvizhukhaAbility.values();
			if (index < dvizh.length) {
				DvizhukhaManager.use(player, dvizh[index]);
			}
			return;
		}
		if (action == SUMMON) {
	...
	private static void toggleStand(ServerPlayerEntity player) {
		...
		if (!canHaveStand(player)) {
			player.sendMessage(Text.translatable("message.crazydiamond.need_stand"), true);
			return;
		}
	...
	public static boolean canHaveStand(ServerPlayerEntity player) {
		return !REQUIRE_STAND_ARROW || player.isCreative() || PlayerPowers.get(player.getServer()).hasStand(player.getUuid());
	}
